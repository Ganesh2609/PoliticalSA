...
def get_data_loaders(train_dir:str, val_dir:str, test_dir:str, batch_size:int, num_workers:int=12, prefetch_factor:int=2, seed:int=42):
    
    torch.manual_seed(seed)

    train_dataset = PoliticalSentimentAnalysis(train_dir)
    #val_dataset = PoliticalSentimentAnalysis(val_dir)
    test_dataset = PoliticalSentimentAnalysis(test_dir)
    ...
    val_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True, persistent_workers=True, prefetch_factor=prefetch_factor)
